...
export function xyFor(stringIndex, fretIndex) {
  // 1..6 for string (left→right), 0..5 for fret (0=nut line)
  ...
  return {
    x: MARGINS.left + stringGap * (stringIndex - 1),
    y: MARGINS.top + fretGap * fretIndex,
  };
}

export default function GuitarTab({ width=458, height=574, thin=3, nut=12, children }) {
  const innerW = TAB_VIEWBOX.w - (MARGINS.left + MARGINS.right);
  const innerH = TAB_VIEWBOX.h - (MARGINS.top + MARGINS.bottom);
  ...
  return (
    <svg xmlns="http://www.w3.org/2000/svg"
      ...
      {/* nut */}
      {line({ x1: x0, y1: y0, x2: x1, y2: y0, strokeWidth: nut })}
